...
import { captureError, withSpan } from "@/shared/telemetry/index.ts";
...
/**
 * Decoded shape of the `MerchantEntry` struct returned by the registry's
 * `getMerchant(merchantId, terminalId)` view. Mirrors the Solidity tuple in
 * `W3SPayMerchantRegistry.sol`; `RawMerchantEntry` in
 * `list-merchant-entries.ts` is a duplicate kept private to that module's
 * UI-facing transform.
 */
...
export interface MerchantRegistryWriteOptions {
  ...
  /**
   * Optional inclusion oracle for the contract write — see
   * `@/sdk/contracts`'s `watchTransaction`. For the merchant
   * registry, callers build this via `makeMerchantEffectOracle` below.
   */
 ...
 *
 * Read errors propagate; the watcher catches them as "non-fatal poll error
 * (continuing)" and the stall watchdog still fires if reads never recover.
 *
 * Trivial satisfaction (the predicate already holds before submission) is
 * not a concern here: `writeContract`'s dry-run reverts on every shape of
 * "no-op write" the registry rejects (e.g. delete of an unknown merchant,
 * register of an existing one), so the oracle is only polled when the
 * pre-state is in the predicate's complement.
 */
