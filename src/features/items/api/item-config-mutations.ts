/**
 * Item-config publish flow — the write half of the old `useItemConfigs`.
 *
 * Publishing is a multi-step progress flow (per config: Bulletin upload →
 * chain `upsertItemConfig`), so it is NOT modeled as a bare `useMutation`
 * (which exposes only `isPending`, not per-step progress and not a
 * non-throwing result union). Instead this hook tracks `publishInFlight`
 * + `publishProgress` locally, drives the feedback-store toasts and the
 * `publish-item-configs` telemetry journey exactly as before, optimistically
 * patches the registry query cache so the dirty diff flips green
 * immediately, and `invalidateQueries` reconciles against the chain on
 * success (the mutation-style cache effect the plan calls for).
 *
 * Reads drafts from `use-item-drafts-store` and the published snapshots
 * from the registry query cache at call time (no stale closures).
 */

import { useCallback, useState } from "react";

import { withSpan } from "@/shared/telemetry/index.ts";
...
import { queryRoots } from "@shared/api/keys.ts";
import {
...
/** Patch one published snapshot into the registry query cache. */
